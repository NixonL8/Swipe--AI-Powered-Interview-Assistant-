...
import type { RootState } from '@/app/storage'
...
import { generateQuestionsOrFallback } from '@/services/ai'
...
function validateProfileField(
  ...
  if (field === 'email') {
    ...
    if (!emailRegex.test(trimmed)) {
      return {
        ok: false,
        message: 'The email you entered isn’t valid. Please check and enter it again.',
      }
    }
  }
  ...
  if (missing.length > 0) {
    dispatch(
      appendChatMessage({
        ...
        message: createMessage(
          'assistant',
          `Before we begin, I need ${formatMissingFields(missing)}. Let's do it step by step - please provide your ${missing[0]}.`,
          'system',
